interface Particle {
  ...
  type: "heart" | "dot"; wobble: number; wobbleSpeed: number; layer: 1 | 2 | 3; color: "plum" | "gold";
}

export function mountParticleField(canvas: HTMLCanvasElement): () => void {
  ...
  function spawnParticle(i: number): Particle {
    ...
    const color: "plum" | "gold" = layer === 2 ? "gold" : "plum";
    return {
      ...
      opacity: layer === 1 ? Math.random() * 0.14 + 0.05
        : layer === 2 ? Math.random() * 0.18 + 0.06
  ...
  function drawHeart(ctx: CanvasRenderingContext2D, x: number, y: number, size: number, color: "plum" | "gold") {
    ctx.save();
    ctx.translate(x, y);
  ...
  function drawDot(ctx: CanvasRenderingContext2D, x: number, y: number, size: number, color: "plum" | "gold") {
    const r = size * 0.35;
  ...
  function tick() {
    ...
    for (const layer of [3, 2, 1] as const) {
      for (const p of particles) {
        ...
        else drawDot(ctx!, p.x, p.y, p.size, p.color);
        ctx!.restore();
      }
    }
