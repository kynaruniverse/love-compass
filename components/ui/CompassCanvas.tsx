  ...
  useEffect(() => {
    ...
    function drawAurora(w: number, h: number, t: number) {
      ribbons.forEach((r) => {
        ...
        ctx!.beginPath();
        for (let x = 0; x <= w; x += 6) {
          // t is seconds; wave moves slowly left-to-right
    ...
    // Compass rose — unchanged, it looked good
    ...
    for (let i = 0; i < 28; i++) {
      particles.push({
        ...
        speed:   Math.random() * 0.025 + 0.008,  // pixels/sec as fraction of height
    ...
    function drawParticles(w: number, h: number, t: number) {
      particles.forEach((p) => {
        // Gentle horizontal drift using sine, very slow
        const drift = Math.sin(t * 0.15 + p.phase) * 0.015;
        const px = (p.x + drift) * w;
        // Rise slowly upward, wrap around
    ...
    function draw(timestamp: number) {
      ...
      // t is seconds elapsed — clean, predictable
      ...
      // One full rotation every ~78 seconds
